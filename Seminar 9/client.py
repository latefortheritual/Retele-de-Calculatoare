...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            ...
            continue
        if not argument:
            print("EROARE locala: Trebuie sa furnizezi un text. Exemplu: PUBLISH Salut!")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if not este_conectat:
            ...
            continue
        if not argument:
            print("EROARE locala: Trebuie sa furnizezi ID-ul mesajului. Exemplu: DELETE 1")
            continue
        try:
            int(argument)
        except ValueError:
            print(f"EROARE locala: ID-ul '{argument}' nu este un numar intreg valid.")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
